...
def inventory(interface: str, mac: str, ip: str) -> dict:
    ...
    try:
        ...
        for line in text.splitlines():
            ...
            if key.strip().lower() in {"model name", "hardware", "processor", "model"}:
                value = value.strip()
    ...
    return {
        ...
        "cpu_model": cpu_model or "LoongArch64",
        "cpu_cores": int(cpu_cores), "memory_bytes": int(memory_bytes),
...
if command -v nmcli >/dev/null 2>&1 && [[ -n "$iface" ]]; then
    ...
    configured=1
elif [[ -d /etc/netplan && -n "$iface" ]] && command -v netplan >/dev/null 2>&1; then
    mkdir -p /etc/netplan
    if [[ "$MODE" == dhcp ]]; then
        cat >/etc/netplan/99-zos-identity.yaml <<NETPLAN_EOF
network:
  version: 2
  ethernets:
    zos_identity:
      match: {macaddress: "$TARGET_MAC"}
      set-name: "$iface"
      dhcp4: true
NETPLAN_EOF
    else
...
NETPLAN_EOF
        [[ -z "$GATEWAY" ]] || sed -i "/addresses:/a\      routes: [{to: default, via: $GATEWAY}]" /etc/netplan/99-zos-identity.yaml
        [[ -z "$DNS" ]] || sed -i "/addresses:/a\      nameservers: {addresses: [${DNS}]}" /etc/netplan/99-zos-identity.yaml
    fi
    chmod 0600 /etc/netplan/99-zos-identity.yaml
    netplan generate && netplan apply && configured=1
    [[ "$configured" == 1 ]] || failure="Netplan rejected the requested network settings"
...
systemctl disable zos-firstboot-identity.service >/dev/null 2>&1 || true
rm -f /etc/systemd/system/multi-user.target.wants/zos-firstboot-identity.service
rm -f /etc/systemd/system/zos-firstboot-identity.service "$0"
